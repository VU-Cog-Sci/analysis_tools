...
class StarStimOperator(Operator):
	...
	def easy_to_fif(self, fif_file_name = None):
		...
		event_samples = np.concatenate((event_samples, event_samples+1, event_samples+2)) # blowing up the event samples.
		event_values = self.raw_data[:,-2][self.raw_data[:,-2] != 0] 
		event_values = np.tile(event_values, 3) # to accommodate the event_samples that have been blown up.
	...
	def line_filter_fif(self, fif_input_file_name = None, fif_output_file_name = None, notch_freq = 50):
		...
		raw = mne.io.RawFIFF(fif_input_file_name, preload=True)
	...
	def psd_quality_control(self, fif_input_file_name = None, l_freq = 0.2, h_freq = 50 ):
		...
		# events = mne.find_events(raw, stim_channel='STI 014', min_duration = 0.5 / raw.info['sfreq'])
