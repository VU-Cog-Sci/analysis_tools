...
class preprocessing():
	...
	def preproces_run(self, this_run):
		...
		###################################################################################################################
		## DETECT EN INTERPOLATE BLINKS AND MISSING DATA! #################################################################
		...
		# f.add_subplot(2,1,1)
		# plot(lp_hp_c_raw_pupil_diameter[500000:550000])
		# f.add_subplot(2,1,2)
		# plot(lp_c_raw_pupil_diameter[500000:550000])
		...
		pupil_mean_lp2 = np.array(lp_c_raw_pupil_diameter).mean()
		pupil_std_lp2 = lp_c_raw_pupil_diameter.std()
		pupil_zscore_lp2 = (lp_c_raw_pupil_diameter - pupil_mean_lp2) / pupil_std_lp2 # Possible because vectorized.
		...
		try:
			...
			pass
		h5f.createArray(run, 'stimulus_locked_array', stim, 'stimulus locked z-scored pupil diameter low (10Hz) + high passed (0.02Hz), and only low passed (10Hz)')
		...
		try:
			...
			pass
		h5f.createArray(run, 'response_locked_array', resp, 'response locked z-scored pupil diameter low (10Hz) + high passed (0.02Hz), and only low passed (10Hz)')
		...
		if self.experiment == 1:
			...
			try:
				...
				pass
			h5f.createArray(run, 'feedback_locked_array', feed, 'feedback locked z-scored pupil diameter low (10Hz) + high passed (0.02Hz), and only low passed (10Hz)')
		...
		trial_on[:,0] = trial_times['trial_phase_timestamps'][:,0,0] - trial_times['trial_start_EL_timestamp'][0]
		trial_on[:,1] = 0.1
		trial_on[:,2] = np.ones(trial_parameters.shape[0])
		# Stick stim onset
		stim_on = np.zeros((trial_parameters.shape[0],3))
		stim_on[:,0] = trial_times['trial_phase_timestamps'][:,1,0] - trial_times['trial_start_EL_timestamp'][0]
		stim_on[:,1] = 0.1
		stim_on[:,2] = np.ones(trial_parameters.shape[0])
		# Stick stim offset
		stim_off = np.zeros((trial_parameters.shape[0],3))
		stim_off[:,0] = trial_times['trial_phase_timestamps'][:,2,0] - trial_times['trial_start_EL_timestamp'][0]
		stim_off[:,1] = 0.1
		stim_off[:,2] = np.ones(trial_parameters.shape[0])
		# Ramp decision interval:
		decision_ramp = np.zeros((trial_parameters.shape[0],3))
		decision_ramp[:,0] = trial_times['trial_phase_timestamps'][:,1,0] - trial_times['trial_start_EL_timestamp'][0]
		decision_ramp[:,1] = trial_times['trial_phase_timestamps'][:,2,0] - trial_times['trial_phase_timestamps'][:,1,0]
		decision_ramp[:,2] = np.ones(trial_parameters.shape[0])
		
		if self.experiment == 1:
			# Sticks confidence
			conf_on = np.zeros((trial_parameters.shape[0],3))
			conf_on[:,0] = trial_times['trial_phase_timestamps'][:,4,0] - trial_times['trial_start_EL_timestamp'][0]
			conf_on[:,1] = 0.1
			conf_on[:,2] = np.ones(trial_parameters.shape[0])
			# Sticks feedback
			feed_on = np.zeros((trial_parameters.shape[0],3))
			feed_on[:,0] = trial_times['trial_phase_timestamps'][:,5,0] - trial_times['trial_start_EL_timestamp'][0]
			feed_on[:,1] = 0.1
			feed_on[:,2] = np.ones(trial_parameters.shape[0])
		
			bla = list((trial_on, stim_on, stim_off, decision_ramp, conf_on, feed_on))
		
		if self.experiment == 2:
			bla = list((trial_on, stim_on, stim_off, decision_ramp))
		
		try:
			run.regressors.remove()
		except NodeError, LeafError:
			pass
		h5f.createArray(run, 'regressors', bla, 'regressors: (1) stick at trial onset (spacebar press), (2) stick at stim onset (tone), (3) stick at stim offset (response), (4) ramp decision interval, (5) stick at confidence (response), (6) stick at feedback' )
...
class make_dataframe():
	...
	def data_frame(self):
		...
		for i in range(len(run)):
			...
			# This is all downsampled times series. z-scored per run based on ALL measurements:
			pupil_diameter_lp.append(run[i].pupil_data_filtered.read()[4,:])
			...
			# Regressor files:
			regressor_trial_on.append( np.array(run[i].regressors.read()[0])[-omission_indices[i]] )
			regressor_stim_on.append( np.array(run[i].regressors.read()[1])[-omission_indices[i]] )
			regressor_stim_off.append( np.array(run[i].regressors.read()[2])[-omission_indices[i]] )
			regressor_ramp.append( np.array(run[i].regressors.read()[3])[-omission_indices[i]] )
			if self.experiment == 1:
				regressor_conf.append( np.array(run[i].regressors.read()[4])[-omission_indices[i]] )
				regressor_feed.append( np.array(run[i].regressors.read()[5])[-omission_indices[i]] )
		...
		pupil_diameter_lp_joined = np.concatenate(pupil_diameter_lp, axis=1)
		...
		# Seperate arrays (with PPD for individual trials) for every run are joined in the list variables 'ppd', 'ppd_mean' and 'ppd_lin_A'.
		# Later, all the elements in these lists are concatenated in the variables 'ppd_joined', 'ppd_mean_joined' and 'ppd_lin_A_joined' (plus the same for ppd after feedback).   
		...
		for i in range(len(run)):
			...
			# pooled_mean_ppd = ( (bottleneck.nanmean(response_locked_array_joined[hit_indices_joined[0],2500:4997], axis=0)) + (bottleneck.nanmean(response_locked_array_joined[fa_indices_joined[0],2500:4997], axis=0)) + (bottleneck.nanmean(response_locked_array_joined[miss_indices_joined[0],2500:4997], axis=0)) + (bottleneck.nanmean(response_locked_array_joined[cr_indices_joined[0],2500:4997], axis=0)) ) / 4
			pooled_mean_ppd = ( (bottleneck.nanmean(response_locked_array_joined[hit_indices_joined,2500:5000], axis=0) - mean(bpd_joined[hit_indices_joined])) + (bottleneck.nanmean(response_locked_array_joined[fa_indices_joined,2500:5000], axis=0) - mean(bpd_joined[fa_indices_joined])) + (bottleneck.nanmean(response_locked_array_joined[miss_indices_joined,2500:5000], axis=0) - mean(bpd_joined[miss_indices_joined])) + (bottleneck.nanmean(response_locked_array_joined[cr_indices_joined,2500:5000], axis=0) - mean(bpd_joined[cr_indices_joined])) ) / 4
			
			for j in range(response_locked_arrays[i].shape[0]):
				ppd_dum[j] = ( np.dot( pooled_mean_ppd, (response_locked_arrays[i][j,2500:5000])-bpd[i][j] ) / np.dot(pooled_mean_ppd, pooled_mean_ppd)  )
			ppd_lin_A.append(ppd_dum)
		...
		if self.experiment == 1:
			...
			for i in range(len(run)):
				...
				# PPD - Method 3 (linear projection):
				ppd_dum =  np.zeros(feedback_locked_arrays[i].shape[0])
				# pooled_mean_ppd = (bottleneck.nanmean(response_locked_arrays[i][-omission_indices[i],2500:4997], axis=0)) - mean(bpd[i][-omission_indices[i]])
				...
				pooled_mean_ppd_feed = ( (bottleneck.nanmean(feedback_locked_array_joined[hit_indices_joined,1000:2500], axis=0) - mean(bpd_feed_joined[hit_indices_joined])) + (bottleneck.nanmean(feedback_locked_array_joined[fa_indices_joined,1000:2500], axis=0) - mean(bpd_feed_joined[fa_indices_joined])) + (bottleneck.nanmean(feedback_locked_array_joined[miss_indices_joined,1000:2500], axis=0) - mean(bpd_feed_joined[miss_indices_joined])) + (bottleneck.nanmean(feedback_locked_array_joined[cr_indices_joined,1000:2500], axis=0) - mean(bpd_feed_joined[cr_indices_joined])) ) / 4
		...
		ppd_lin_A_joined = np.concatenate(ppd_lin_A)
		...
		if self.experiment == 1:
			...
			ppd_feed_lin_A_joined = np.concatenate(ppd_feed_lin_A)
		...
		ppd_lin_A_RT_corrected = np.zeros(len(ppd_lin_A_joined))
		for i in range(len(ppd_lin_A_joined)):
			ppd_lin_A_RT_corrected[i] = (ppd_lin_A_joined[i] * mean_RT_for_correct) / decision_time_joined[i]
		
		###################################################################################################################
		## GET D_PRIME AND CRITERION! #####################################################################################
		
		d_prime_subject, criterion_subject = functions_jw.SDT_measures_per_subject(subject = self.subject, target_indices_joined = target_indices_joined, no_target_indices_joined = no_target_indices_joined, hit_indices_joined = hit_indices_joined, fa_indices_joined = fa_indices_joined)
		d_prime_subject_run, criterion_subject_run = functions_jw.SDT_measures_per_subject_per_run(subject = self.subject, target_indices = target_indices, no_target_indices = no_target_indices, hit_indices = hit_indices, fa_indices = fa_indices)
		...
		if self.experiment == 1:
			d = {
				'test contrast' : pd.Series(test_contrast_joined),
				'd-prime' : pd.Series(d_prime_subject),
				'criterion' : pd.Series(criterion_subject),
				'd-prime_per_run' : pd.Series(d_prime_subject_run),
				'criterion_per_run' : pd.Series(criterion_subject_run),
				'number trials in run' : pd.Series(number_trials_in_run_joined),
				...
				'baseline pupil diameter before feedback' : pd.Series(bpd_feed_joined),
				'phasic pupil diameter' : pd.Series(ppd_lin_A_joined),
				'phasic pupil diameter RT corrected' : pd.Series(ppd_lin_A_RT_corrected),
				'phasic pupil diameter after feedback' : pd.Series(ppd_feed_lin_A_joined)
				}
		if self.experiment == 2:
			d = {
				'test contrast' : pd.Series(test_contrast_joined),
		...
		resp_locked_grand_mean = ( bottleneck.nanmean(response_locked_array_joined[hit_indices_joined,:], axis=0) + bottleneck.nanmean(response_locked_array_joined[hit_indices_joined,:], axis=0) + bottleneck.nanmean(response_locked_array_joined[hit_indices_joined,:], axis=0) + bottleneck.nanmean(response_locked_array_joined[hit_indices_joined,:], axis=0) ) / 4
		...
		# Save to folder called 'saves' located on the desktop:
		if self.experiment == 1:
