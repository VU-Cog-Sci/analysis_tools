...
#        print self.scanTypeDict
    ...
    def registerSession(self, contrast = 't2', FSsubject = None, prepare_register = True, which_epi_target = 0, postFix = None, deskull = True, bb = True, makeMasks = False, maskList = ['cortex','V1','V2','V3','V3A','V3B','V4'], labelFolder = 'label', MNI = True, run_flirt = True):
        """
        ...
        if prepare_register:
            if 'inplane_anat' in self.scanTypeList:
                ...
                mcFirst = MCFlirtOperator( self.runFile(stage = 'processed/mri', run = self.runList[self.scanTypeDict['epi_bold'][which_epi_target]], postFix = postFix ) )
...
#            ppResults = [job_server.submit(mcf.execute,(), (), ("Tools","Tools.Operators","Tools.Sessions.MCFlirtOperator","subprocess",)) for mcf in mcOperatorList]
    ...
    def B0_unwarping(self, conditions, wfs, etl, acceleration, unwarp_direction='y', asym=0.005, f_value=0.5):
        
        """
        https://www.spinozacentre.nl/wiki/index.php/NeuroWiki:Current_developments
        """
        ...
        inputObject = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_phs'][0]])
        outputObject = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_phs'][0]], postFix=['rescaled'])
        extend = round(max(abs(nib.load(inputObject).get_data().ravel())),0)
        fmO = FSLMathsOperator(inputObject=inputObject)
        fmO.configurePi(outputFileName=outputObject, div=extend, mul=3.141592653589793116)
        fmO.execute()
        ...
        #     outputObject = self.runFile(stage = 'processed/mri', run = self.runList[r], postFix=['brain'])
        #     better = BETOperator( inputObject = inputObject )
        #     better.configure( outputFileName = outputObject, f_value=f_value, g_value=0 )
        #     better.execute()
        
        # create mask:
        inputObject = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_mag'][0]])
        outputObject = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_mag'][0]], postFix=['mask'])
        fmO = FSLMathsOperator(inputObject=inputObject)
        fmO.configure(outputFileName=outputObject, **{'-bin': ""})
        fmO.execute()
        
        inputObject = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_mag'][0]], postFix=['mask'])
        outputObject = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_mag'][0]], postFix=['mask'])
        fmO = FSLMathsOperator(inputObject=inputObject)
        ...
        phasevol = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_phs'][0]], postFix=['rescaled'])
        inputObject = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_mag'][0]])
        outputObject = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_phs'][0]], postFix=['rescaled', 'unwrapped'])
        mask = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_mag'][0]], postFix=['mask'])
        ...
        for r in self.conditionDict['B0_anat_phs']:
            ...
            fmO = FSLMathsOperator(inputObject=inputObject)
            fmO.configure(outputFileName=outputObject, **{'-mul': str(1.0/asym)})
            fmO.execute()
        
        # UNWARP:
        
        effective_echo_spacing = ((1000.0 * wfs)/(434.215 * (etl+1))/acceleration) / 1000.0 # should be in seconds
        ...
        for cond in conditions:
            for r in [self.runList[i] for i in self.conditionDict[cond]]:
                ...
                inputObject = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_mag'][0]])
                referenceFileName = self.runFile(stage = 'processed/mri', run = r)
                ...
                referenceFileName = self.runFile(stage = 'processed/mri', run = r)
                transformMatrixFileName = self.runFile(stage = 'processed/mri', run = self.runList[self.conditionDict['B0_anat_mag'][0]], postFix=['trans'], extension='.mat')
                fO = FlirtOperator(inputObject, referenceFileName)
                ...
                command_line = 'fugue -i {} -u {} --loadfmap={} --unwarpdir={} --dwell={} --asym={} -m'.format(
                    self.runFile(stage = 'processed/mri', run = r),
