...
class HexagonalSaccadeAdaptationSession(object):
    ...
    def trial_selection(self, alias, no_std_vel_cutoff=3, amp_range=[5, 15], degree_cutoff=3, initials='unkown', trial_rejection_logger=[], analyze_eye='L'):
        ...
        saccade_table = saccade_table[
            saccade_table.eye == analyze_eye].set_index('trial')

        # initialize outcome array
        trials2use = np.ones(saccade_table.shape[0])

        # 1. deselect trial when the blink onset was within trial phase 2 of
        # that trial:
        phase_2_start_times = np.array(
            trial_phases.trial_phase_EL_timestamp[trial_phases.trial_phase_index == 2])
        phase_2_end_times = np.array(
            trial_phases.trial_phase_EL_timestamp[trial_phases.trial_phase_index == 3])
        ...
        # 2. deselect trial when: peak velocity more than no_std_vel_cutoff std
        # off the mean
        for bi in range(self.nr_blocks):
            trials_this_block = np.arange(
                bi * self.nr_trials_per_block, bi * self.nr_trials_per_block + self.nr_trials_per_block)
            mean_vel = np.median(
                saccade_table.peak_velocity[trials_this_block])
            std_vel = no_std_vel_cutoff * \
                np.std(saccade_table.peak_velocity[trials_this_block])
        ...
        # # 3a. deselect trial when amplitude falls within the 'amp_range'
        ...
        # 3b. deselect trial when amplitude falls within 3 std of average
        # amplitude per block
        amp_avg_blocks = [np.mean(saccade_table.expanded_amplitude[
                                  i * self.nr_trials_per_block:i * self.nr_trials_per_block + self.nr_trials_per_block]) for i in np.arange(8)]
        amp_std_blocks = [np.std(saccade_table.expanded_amplitude[
                                 i * self.nr_trials_per_block:i * self.nr_trials_per_block + self.nr_trials_per_block]) for i in np.arange(8)]
        for i, amp in enumerate(saccade_table.expanded_amplitude):
        ...
        # because the original dot locations were not saved to the parameters
        # file, I recreate them here:
        physical_screen_size = (40, 30)
        physical_screen_distance = 50
        screen_resolution = (800, 600)
        amplitude = self.saccade_amplitude
        n_directions = 6
        screen_height_degrees = 2.0 * 180.0 / np.pi * \
            math.atan(
                (physical_screen_size[1] / 2.0) / physical_screen_distance)
        pixels_per_degree = (screen_resolution[1]) / screen_height_degrees
        dot_directions = (
            np.arange(n_directions) * 2.0 * np.pi) / float(n_directions)
        dot_xy = np.array([[-np.sin(d), -np.cos(d)] for d in dot_directions])
        dot_xy_screen = amplitude * pixels_per_degree * dot_xy

        # now that we know the dot positions, we can compute the actual
        # positions by subtracting the offsets and deselect unwanted trials
        gaze_offset = np.zeros(saccade_table.shape[0])
        for ti in range(np.size(saccade_table, 0)):
            actual_dot_position = [dot_xy_screen[params.dot_index[ti].astype(int)][0] + params.stim_offset_pre_sacc_x[
                ti], dot_xy_screen[params.dot_index[ti].astype(int)][1] + params.stim_offset_pre_sacc_y[ti]]
            gaze_offset[ti] = LA.norm([actual_dot_position[0] - (np.array(saccade_table.expanded_start_point[ti])[0] - 400),
                                       actual_dot_position[1] - (np.array(saccade_table.expanded_start_point[ti])[1] - 300)]) / params.pixels_per_degree[ti]
        trials2use[gaze_offset > degree_cutoff] = 0
        c_gaze = int(
            saccade_table.shape[0] - sum(trials2use) - c_blink - c_vel - c_amp)

        # log amount of trials rejected. NOTE: the counters do not include
        # trials that were already rejected by the former step, so reflect only
        # ADDITIONAL AMOUNT OF TRIALS rejected by ... procedure
        if trial_rejection_logger != []:
            trial_rejection_logger.info('Trials rejected for subject ' + initials + ' : total ' + str(c_blink + c_amp + c_vel + c_gaze) + ' | blinks ' + str(c_blink) + ' | amplitude ' + str(
                c_amp) + ' | velocity ' + str(c_vel) + ' [range: ' + str(vel_range) + ' ] gaze ' + str(c_gaze) + '  [offset: ' + str(degree_cutoff) + ' ] degrees) in  ' + alias)

        # also return vel_range for plotting purposes, as this is different for
        # every participant (because of std +/- mean computation)
    ...
    def velocity_for_saccades(self, alias):
        ...
        with pd.get_store(self.ho.inputObject) as h5_file:
            saccade_table = h5_file['%s/saccades_per_trial' % alias]
        signal_profiles = np.ones(
            (saccade_table.shape[0], self.velocity_profile_duration)) * np.nan
        for s in saccade_table.iterrows():
            index = s[0]
            s = s[1]  # lose the index
            if s['raw_start_time'] != 0.0:
                int_signal = self.ho.signal_from_trial_phases(trial_nr=s['trial'], trial_phases=[
                                                              1, 3], alias=alias, signal='vel', requested_eye=s['eye'], time_extensions=[s['raw_start_time'] - 10, 350])
                if (np.__version__.split('.')[0] == 1) and (np.__version__.split('.')[1] > 6):
                    signal_profiles[index, :np.min((self.velocity_profile_duration, int(s['raw_duration']) + 20))] = LA.norm(
                        int_signal, axis=1)[:np.min((self.velocity_profile_duration, int(s['raw_duration']) + 20))]
                else:
                    signal_profiles[index, :np.min((self.velocity_profile_duration, int(s['raw_duration']) + 20))] = np.array(
                        [LA.norm(ints) for ints in np.array(int_signal)])[:np.min((self.velocity_profile_duration, int(s['raw_duration']) + 20))]
            else:
                pass
        s_name = 'velocity'
        self.ho.data_frame_to_hdf(alias=alias, name='saccade_%s_profiles' %
                                  s_name, data_frame=pd.DataFrame(np.array(signal_profiles)))
    ...
    def gaze_for_saccades(self, alias):
        ...
        with pd.get_store(self.ho.inputObject) as h5_file:
            saccade_table = h5_file['%s/saccades_per_trial' % alias]
            signal_profiles = np.ones(
                (saccade_table.shape[0], self.signal_profile_duration, 2)) * np.nan
        for s in saccade_table.iterrows():
            index = s[0]
            s = s[1]  # lose the index
            if s['raw_start_time'] != 0.0:
                int_signal = self.ho.signal_from_trial_phases(trial_nr=s['trial'], trial_phases=[
                                                              1, 3], alias=alias, signal=s_name, requested_eye=s['eye'], time_extensions=[s['raw_start_time'] - 10, 350])
                signal_profiles[index, :np.min((self.signal_profile_duration, int(
                    s['raw_duration']) + 20))] = int_signal[:np.min((self.signal_profile_duration, int(s['raw_duration']) + 20))]
            else:
                pass
        self.ho.data_frame_to_hdf(
            alias=alias, name='saccade_%s_profiles' % s_name, data_frame=pd.Panel(signal_profiles))
    ...
    def plot_velocity_profiles_for_blocks(self, bins_per_block=5):
        ...
        for i, alias in enumerate(self.conditions.keys()):
            ...
            for j in range(self.nr_blocks):
                ...
                for e in 'LR':
                    ...
                    if str(j) + '_' + e in wp.items:
                        ...
                        for bin_nr in range(bins_per_block):
                            bin_data = this_block_data[
                                bin_nr * bin_size:(bin_nr + 1) * bin_size]
                            ss.plot(bn.nanmean(bin_data, axis=0), c=c, linewidth=1.75,
                                    alpha=0.2 + (0.8 * bin_nr * 1.0 / bins_per_block), linestyle=ls)
                    else:
                ...
                sub_plot_counter += 1
        pl.savefig(
            os.path.join(self.base_directory, 'figs', 'blocked_velocity_profiles.pdf'))

    def signals_per_block(self, signal_type='velocity'):
        """docstring for velocity_signals_per_block"""
        panel_list = []
        for alias in self.conditions.keys():
            eyes = [self.ho.eye_during_trial(
                tbi[0], alias=alias) for tbi in self.block_trial_indices]
            with pd.get_store(self.ho.inputObject) as h5_file:
                saccade_signal_profiles_table = h5_file[
                    '%s/saccade_%s_profiles' % (alias, signal_type)]
                saccades_per_trial_table = h5_file[
                    '%s/saccades_per_trial' % alias]
            nr_eye_blocks = len("".join(eyes))
            alias_signal_profiles = np.ones(
                (nr_eye_blocks, self.nr_trials_per_block, self.signal_profile_duration)) * np.nan
            eye_block_counter = 0
            labels = []
            for i, be in enumerate(eyes):  # loop across blocks, for the eyes
                for e in be:  # loop across the eyes in the string "be"
                    which_trials_this_block_eye = (
                        saccades_per_trial_table['block'] == i) * (saccades_per_trial_table['eye'] == e)
                    alias_signal_profiles[eye_block_counter] = np.array(
                        saccade_signal_profiles_table[which_trials_this_block_eye])
                    labels.append({'block': i, 'eye': e})
                    eye_block_counter += 1
            wp = pd.Panel(alias_signal_profiles, items=[str(
                l['block']) + '_' + l['eye'] for l in labels], major_axis=np.arange(self.nr_trials_per_block))
    ...
    def amplitudes_first_last_bino_block(self, which_amplitude='raw_amplitude', acceptance_amplitude_range=[4.0, 14.0], analyze_eye='L'):
        ...
        for alias in self.conditions.keys():
            ...
            first_block_amplitudes = np.array([saccade_table[which_amplitude][
                                              i * self.nr_trials_per_block:(i + 1) * self.nr_trials_per_block] for i in [0, 1]])
            # take out non-relevant trials, such as zero amplitude or such
            first_block_mask = ((first_block_amplitudes > acceptance_amplitude_range[
                                0]) * (first_block_amplitudes < acceptance_amplitude_range[1])).prod(axis=0, dtype=bool)
            first_block_amplitudes = first_block_amplitudes[
                :, first_block_mask]
            first_block_eyes = np.array(
                [saccade_table['eye'][i * self.nr_trials_per_block] for i in [0, 1]])

            last_block_amplitudes = np.array([saccade_table[which_amplitude][
                                             nr_saccs_in_session - self.nr_trials_per_block * (i + 1):nr_saccs_in_session - self.nr_trials_per_block * i] for i in [1, 0]])
            # take out non-relevant trials, such as zero amplitude or such
            last_block_mask = ((last_block_amplitudes > acceptance_amplitude_range[
                               0]) * (last_block_amplitudes < acceptance_amplitude_range[1])).prod(axis=0, dtype=bool)
            last_block_amplitudes = last_block_amplitudes[:, last_block_mask]
            last_block_eyes = np.array([saccade_table['eye'][
                                       nr_saccs_in_session - self.nr_trials_per_block * (i + 1)] for i in [1, 0]])
            ...
            first_adapted_eye = saccade_table['eye'][
                2 * self.nr_trials_per_block + 1]
        ...
        for (sess, alias) in enumerate(self.conditions.keys()):
            ...
            spine_shift(s)
            for j in range(2):
                s.plot(block_amps[sess][j][0], 'b' +
                       [':', '--'][j], linewidth=1.0, alpha=0.5)
                s.plot(block_amps[sess][j][1], 'k' +
                       [':', '--'][j], linewidth=1.0, alpha=0.5)
                # s.set_ylim([5,13])
            s.set_title(alias)
        pl.savefig(os.path.join(self.base_directory, 'figs',
                                'first_last_bino_block_%s_amps.pdf' % which_amplitude))
        ...
        for (sess, alias) in enumerate(self.conditions.keys()):
            ...
            simpleaxis(s)
            spine_shift(s)
            pl.hist(block_diffs[sess][0], color='g', alpha=0.5, histtype='step', bins=100,
                    range=[-3, 3.1], label='Pre', normed=True, cumulative=True, linewidth=3.0)
            pl.hist(block_diffs[sess][1], color='r', alpha=0.5, histtype='step', bins=100,
                    range=[-3, 3.1], label='Post', normed=True, cumulative=True, linewidth=3.0)
        ...
        pl.savefig(os.path.join(self.base_directory, 'figs',
                                'first_last_bino_block_%s_diffs_dist.pdf' % which_amplitude))
        ...
        for (sess, alias) in enumerate(self.conditions.keys()):
            ...
            simpleaxis(s)
            spine_shift(s)
            pl.plot(block_diffs[sess][0], color='g',
                    label='Pre', alpha=0.5, linewidth=3.0)
            pl.plot(block_diffs[sess][1], color='r',
                    label='Post', alpha=0.5, linewidth=3.0)
            ...
            if leg:
                ...
                for (j, l) in enumerate(leg.get_lines()):
                    l.set_linewidth(3.5)  # the legend line width
        pl.savefig(os.path.join(self.base_directory, 'figs',
                                'first_last_bino_block_%s_diffs.pdf' % which_amplitude))

        diff_fr = np.zeros((2, 2, 2))
        for (sess, alias) in enumerate(self.conditions.keys()):
            for j in [0, 1]:
                diff_fr[sess, j] = (
                    np.mean(block_diffs[sess][j]), np.std(block_diffs[sess][j]))
    ...
    def fit_adaptation_timecourse_one_block_powerlaw(self, data, trials_okay=[]):
        ...
        if trials_okay == []:
            trials_okay = np.arange(self.nr_trials_per_block)

        (ar, br) = polyfit(np.log10(np.arange(1, self.nr_trials_per_block + 1))
                           [trials_okay], np.log10(data)[trials_okay], 1)
        xr = polyval(
            [ar, br], np.log10(np.arange(1, self.nr_trials_per_block + 1)))
    ...
    def fit_adaptation_timecourse_one_block_exponential(self, data, trials_okay=[]):
        ...
        (ar, br) = polyfit(np.log10(
            np.arange(1, self.nr_trials_per_block + 1)[trials_okay]), data[trials_okay], 1)
        xr = polyval(
            [ar, br], np.log10(np.arange(1, self.nr_trials_per_block + 1)))
    ...
    def amplitudes_all_adaptation_blocks(self, measures, analyze_eye='L'):
        ...
        for alias in self.conditions.keys():
            ...
            nr_saccs_in_session = saccade_table.shape[0]
            which_trials_okay, vel_range, amp_range = self.trial_selection(
                alias=alias)

            for which_amplitude in measures:

                # fit powerlaw to first block to generate baseline
                res = self.fit_adaptation_timecourse_one_block_powerlaw(np.array(saccade_table[which_amplitude][(
                    saccade_table['block'] == 0)]), which_trials_okay[0:self.nr_trials_per_block])
                baseline = res[5]
                ...
                for i in range(self.nr_blocks):
                    ...
                    # average the two eyes together for these trials
                    if (saccade_table['block'] == i).sum() > self.nr_trials_per_block:
                        ad_bl_tr.append(((np.array(saccade_table[which_amplitude][(saccade_table['block'] == i) * (saccade_table['eye'] == 'R')]) + np.array(
                            saccade_table[which_amplitude][(saccade_table['block'] == i) * (saccade_table['eye'] == 'L')])) / 2.0))  # -baseline)
                    else:
                        ad_bl_tr.append(np.array(
                            saccade_table[which_amplitude][(saccade_table['block'] == i)]))  # -baseline)
                    # do some fitting of power-law
                    f_ad_bl_tr.append([self.fit_adaptation_timecourse_one_block_powerlaw(np.array(ad_bl_tr[i]), which_trials_okay[i * self.nr_trials_per_block:i * self.nr_trials_per_block + self.nr_trials_per_block]),
                                       self.fit_adaptation_timecourse_one_block_exponential(np.array(ad_bl_tr[i]), which_trials_okay[i * self.nr_trials_per_block:i * self.nr_trials_per_block + self.nr_trials_per_block])])
                ...
                for i in range(len(ad_bl_tr)):
                    s1.plot(self.nr_trials_per_block * i + np.arange(self.nr_trials_per_block)[which_trials_okay[i * self.nr_trials_per_block:i * self.nr_trials_per_block + self.nr_trials_per_block]], ad_bl_tr[
                            i][which_trials_okay[i * self.nr_trials_per_block:i * self.nr_trials_per_block + self.nr_trials_per_block]], self.all_block_colors[i] + 'o', mew=1.75, alpha=0.875, mec='w', ms=6)
                    s1.plot(self.nr_trials_per_block * i + np.arange(self.nr_trials_per_block),
                            f_ad_bl_tr[i][0][-1], self.all_block_colors[i] + '--', alpha=0.75, linewidth=4.75)
                    s1.plot(self.nr_trials_per_block * i + np.arange(self.nr_trials_per_block),
                            f_ad_bl_tr[i][1][-1], self.all_block_colors[i] + ':', alpha=0.75, linewidth=4.75)
                simpleaxis(s1)
                spine_shift(s1)

                s1.set_xticks(np.arange(
                    0, self.nr_trials_per_block * self.nr_blocks, self.nr_trials_per_block))
                ...
                if which_amplitude != 'peak_velocity':
                    ...
                    s1.axis(
                        [-20, self.nr_trials_per_block * self.nr_blocks + 20, amp_range[0], amp_range[1]])
                    ...
                    s1.axis(
                        [-20, self.nr_trials_per_block * self.nr_blocks + 20, vel_range[0], vel_range[1]])
                s1.set_xlabel('trials within blocks, ' + alias)
                s1.set_title(alias + '\nTrials + fit')

                pl.savefig(os.path.join(self.base_directory, 'figs',
                                        'adap_time_course_%s_%s.pdf' % (alias, which_amplitude)))
