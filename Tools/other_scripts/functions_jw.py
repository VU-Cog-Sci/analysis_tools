...
##### GENERAL ######
####################
def pupil_IRF(t=0, s=1.0/(10**26), n=10.1, tmax=930):
    ...
    h = ( (s) * (t**n) * (math.e**((-n*t)/tmax)) )
    return(h)


def create_box_regressors(times, dur, len_run, IRF, normalize=True):
    ...
    for i in range(times.shape[0]):
        if normalize == True:
            regr[times[i]:times[i]+dur[i]] = np.linspace(1.0/dur[i],1.0/dur[i],dur[i])
        else:
            regr[times[i]:times[i]+dur[i]] = np.linspace(1.0,1.0,dur[i])
...
def confidence_barplot(subject, hit, fa, miss, cr, p1, p2, p3, values=False):
    ...
    if values == True:
        label_diff(ax,0,1,sig1,X,MEANS, SEMS, values = True)
        label_diff(ax,1,2,sig2,X,MEANS, SEMS, values = True)
        label_diff(ax,2,3,sig3,X,MEANS, SEMS, values = True)
    if values == False:
        label_diff(ax,0,1,sig1,X,MEANS, SEMS)
        label_diff(ax,1,2,sig2,X,MEANS, SEMS)
        label_diff(ax,2,3,sig3,X,MEANS, SEMS)
